...
public class CircleTextImageView extends ImageView {
    ...
    private void setup() {
        ...
        mBorderRadius = Math.min((mBorderRect.height() - mBorderWidth) / 3.0f, (mBorderRect.width() - mBorderWidth) / 3.0f);

        mDrawableRect.set(mBorderRect);
        if (!mBorderOverlay) {
            mDrawableRect.inset(mBorderWidth, mBorderWidth);
        }
        mDrawableRadius = Math.min(mDrawableRect.height() / 3.0f, mDrawableRect.width() / 3.0f);
